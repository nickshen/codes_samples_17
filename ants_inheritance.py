    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.true_queen == True:
                pass #take no action if selected is true/real QueenAnt
            elif self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container: #container, remove container
                    self.ant = self.ant.ant
                else:
                    self.ant = None
                insect.place = None
            else: # direct removal of contained ant
            ...
            insect.place = None # set insect place to none when removed
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        target_place, found_bee = self.place, None
        distance = 0
        while not found_bee:
            if target_place == None or target_place is hive:
                return None

            if distance >= self.min_range and distance <= self.max_range:
                found_bee = random_or_none(target_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4A
        if amount >= self.armor:  #if FireAnt is projected to die, explode!
            for bee in list(self.place.bees):
                Insect.reduce_armor(bee, self.damage)
        Insect.reduce_armor(self, amount)  #regardless of projected outcome, ant takes damage
        # END Problem 4A
...
class WallAnt(Ant):
    ...
    def action(self, colony):
        pass #walls do nothing, no action
# The WallAnt class
# END Problem 5A


class AreaOfEffectAnt(Ant):
    """
    Specialized sub-class of Ant that holds the definition for damage_all(), which deals damage to all Bees
    in self.space/specified area. Damage_all() is not included in Ant because most Ant subclasses don't need damage_all().
    
    Inherited by NinjaAnt and TankAnt for use in action() methods, acts as an interface.
    """
    def damage_all(self, colony):
        for bee in list(self.place.bees):
            Insect.reduce_armor(bee, self.damage) #damage all bees in self.place


class NinjaAnt(AreaOfEffectAnt, Ant):
...
class ScubaThrower(ThrowerAnt):
    name = 'Scuba'
    food_cost = 6
    watersafe = True # the important distinction
    implemented = True
...
class TankAnt(AreaOfEffectAnt, BodyguardAnt):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    queen_exists = False
    implemented = True   # Change to True to view in the GUI
    rallied = set() # Store rallied Ant objects, using set for constant look-up. Order does not matter.
    # END Problem 9

    def __init__(self, armor=1):
        # BEGIN Problem 9
        Insect.__init__(self, armor)
        if QueenAnt.queen_exists:
            self.true_queen = False
        else:
            self.true_queen = True
            QueenAnt.queen_exists = True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.true_queen == False:
            self.reduce_armor(self.armor) # if not True Queen, dispose of this QueenAnt
        else:
            ThrowerAnt.action(self, colony)
            self.rally_troops(colony) # rally fellow ants!!
        # END Problem 9

    def rally_troops(self, colony):
        current_place = self.place.exit
        while current_place != None:
            current_ant = current_place.ant
            self.rally(current_ant)

            # Check if ant is container..
            if hasattr(current_ant, 'container') and current_ant.container:
                contained_ant = current_ant.ant
                self.rally(contained_ant)
            current_place = current_place.exit

    def rally(self, target_ant):
        if target_ant and target_ant not in self.rallied: # If an ant is contained in container ant
            target_ant.damage = target_ant.damage *2
            self.rallied.add(target_ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.true_queen:
            bees_win() # game is over because True Queen has died
        # END Problem 9
...
def make_slow(action):
    ...
    # BEGIN Problem EC
    def slowed(duration):
        def try_action(colony):
            nonlocal duration
            if duration <= 0:
                return action(colony)
            elif colony.time%2 == 0:
                duration -= 1
                return action(colony)
            else:
                duration -= 1
